...
{
   public class MinDecimalTextBox:TextBox
    {
        public MinDecimalTextBox()
        {
            this.KeyPress += DecimalTextBox_KeyPress;
            this.TextChanged += DecimalTexBox_TextChanced;
            this.Text = MinValue.ToString();
        }

        private void DecimalTexBox_TextChanced(object sender, EventArgs e)
        {
            string enteredStr = (sender as System.Windows.Forms.TextBox).Text;//girilen metni al.
            if (String.IsNullOrWhiteSpace(enteredStr))
            {
                (sender as System.Windows.Forms.TextBox).Text = Convert.ToString(MinValue);
            }
            else
            {
                this.Text = enteredStr;
            }
        }
